...
def collect(directory: str, heuristic: Heuristic):
    for file in _get_files(directory):
        file = _get_json_file(file, heuristic)
        if not file.exists():
            print(file, 'missing')
            return
        if len(file.read_text()) < 10:
            print(file, 'does not have text')
...
if __name__ == '__main__':
    ...
    run_all_experiments(INPUT_DIRS[1])
    # collect_to_csv(INPUT_DIRS[1], ALL_HEURISTICS[0])
    # count_missing()
    # _run_9x9()